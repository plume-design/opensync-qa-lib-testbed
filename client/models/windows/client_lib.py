...
class ClientLib(ClientLibGeneric):
    ...
    def hw_info(self, **kwargs):
        ...
        result = f'model_name: {self.run_command(model_name, **kwargs)[1]}; ' \
    ...
    def get_eth_info(self, timeout=5, **kwargs):
        ...
        for iface_alias in self.get_eth_iface(force=True):
            ...
            interface_status = self.get_stdout(self.strip_stdout_result(self.run_command(
                f'"(Get-NetAdapter -Name "{iface_alias}").Status"', **kwargs)))
    ...
    def get_wlan_information(self, **kwargs):
        ...
        get_wlan_driver_info_cmd = f'Get-NetAdapter -Name "{iface_alias}" | ' \
                                   f'Select Name, DriverDescription, DriverVersion, DriverDate, DriverProvider'
        get_ip_cmd = f'(Get-NetIPAddress -AddressFamily IPv4 -InterfaceAlias "{iface_alias}").IPAddress'
        get_802_11ac_support_cmd = f'(Get-NetAdapterAdvancedProperty -Name "{iface_alias}" ' \
                                   f'-RegistryKeyword IEEE11nMode).RegistryValue'

        is_802_11ac_supported = self.get_stdout(
            self.strip_stdout_result(self.run_command(get_802_11ac_support_cmd, **kwargs))) == '2'

        wlan_info = {'wlan': {iface_alias: {
    ...
    def eth_connect(self, pod, port_alias=None, ifname=None, dhclient=True, ipv4=True,
                    ipv6=False, ipv6_stateless=False, **kwargs):
        """
    ...
    def start_dhcp_client(self, ifname, cf=None, ipv4=True, ipv6=False, ipv6_stateless=False,
        ...
        if ipv4:
            ...
            ipv4_granted = wait_for(partial(self.get_ip_address, ifname, "IPv4"), timeout=timeout + 20, tick=1.)[0]
            if not ipv4_granted:
        ...
        if ipv6:
            ipv6_granted = wait_for(partial(self.get_ip_address, ifname, "IPv6"), timeout=timeout + 20, tick=1.)[0]
            if not ipv6_granted:
    ...
    def connect(self, ssid=None, psk=None, ifname=None, bssid=None, key_mgmt=None, timeout=60,  # noqa: C901
                ...
                ipv6_stateless=False, wps=False, proto='RSN', pmf_mode='enabled', retry=3, **kwargs):  # noqa: C901
        """
        ...
        assert wait_for(partial(self._is_wifi_connected, ifname), 60, 5.0)[0], "Client is not connected to WiFi"

        assert wait_for(partial(self._is_gateway_reachable, ifname), 60, 5.0)[0], "Default gateway is not reachable"
    ...
    def _get_ipv4_gateway(self, ifname):
        cmd = f'Get-NetRoute -DestinationPrefix "0.0.0.0/0" -ifAlias "{ifname}" | ' \
    ...
    def _create_windows_wifi_profile(self, ssid, psk, key_mgmt):
        cmd = f'$PW = ConvertTo-SecureString "{psk}" -AsPlainText -Force; ' \
              f'Set-WiFiProfile -ProfileName {ssid} -ConnectionMode manual ' \
              f'-Authentication {self._parse_key_mgmt_to_proper_authentication(key_mgmt)} ' \
