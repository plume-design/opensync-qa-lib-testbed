...
from lib_testbed.generic.client.client_config import TBCFG_CLIENT_DEPLOY
...
class ClientLib(ClientBase):
    ...
    @property
    def key_mgmt(self):
        default_key_mgmt = getattr(self, 'default_key_mgmt', None)
        if not default_key_mgmt:
            self.default_key_mgmt = self.set_default_key_mgmt()
        return getattr(self, 'default_key_mgmt')

    def set_default_key_mgmt(self):
        default_key_mgmt = 'WPA-PSK'
        client_capab = self.get_wlan_information()[1]
        client_wpa3_capab = client_capab['wlan'][self.wlan_iface].get('wpa3', False)
        if not client_wpa3_capab:
            return default_key_mgmt
        # If gateway has a 6g band set SAE as default key_mgmt for Wi-Fi client connection
        supported_bands = self.config['Nodes'][0].get('capabilities', {}).get('supported_bands', '')
        return 'SAE' if '6G' in supported_bands else default_key_mgmt
    ...
    def info(self, **kwargs):
        ...
        all_info = dict(ChainMap(arch_info, image_version, client_model, chariot_info, eth_info, wlan_info, bt_info,
                                 os_info, hostname.copy()))
    ...
    def get_iw_supported_ciphers(iw_info):
        iw_out = iw_info.split("Supported Ciphers:")[1]
        supported_ciphers = []
        for line in iw_out.splitlines():
            line = line.strip()
            if not line:
    ...
    def get_bt_info(self, **kwargs) -> Tuple[int, Dict[str, Dict[str, Dict[str, Union[str, List[str]]]]], str]:
        ...
        response = self.get_stdout(self.strip_stdout_result(self.run_command('yes | btmgmt info', **kwargs)),
                                   skip_exception=True)
        # RegExp match based on BlueZ tools/btmgmt.c:info_rsp()
        for match in re.finditer(r"(?P<interface>hci\d+):\tPrimary controller\n"
        ...
        if kwargs.get('new_tools'):
            ...
            if iface:
                if iface not in bt['bt']:
                    raise ValueError(f'Specified Bluetooth interface "{iface}" is not available ({", ".join(bt["bt"])})'
                                     )
            else:
    ...
    def get_wlan_iface(self, **kwargs):
        ...
        iface = self.wlan_iface if hasattr(self, 'wlan_iface') and not force else self.get_stdout(
    ...
    def get_eth_iface(self, **kwargs):
        ...
        force = kwargs.pop('force', False)
        # Store interface name to avoid additional ssh calls
        iface = self.eth_iface if hasattr(self, 'eth_iface') and not force else self.get_stdout(
            self.strip_stdout_result(self.run_command('ls /sys/class/net | grep "et\|en"', **kwargs)),  # noqa
            skip_exception=True).split('\n')[0]
    ...
    def eth_connect(self, pod, port_alias=None, ifname=None, dhclient=True,
                    ipv4=True, ipv6=False, ipv6_stateless=False, **kwargs):
        """
        ...
        if dhclient:
            dhcp_result = self.start_dhcp_client(ifname, ipv4=ipv4, ipv6=ipv6, ipv6_stateless=ipv6_stateless,
                                                 **kwargs)
    ...
    def fqdn_check(self, count=1, v6=False, **kwargs):
        ...
        result = self.run_command(f"dig -t {domain_type} {dns} {fqdn_check_domain} +noedns | grep -v '^;' "
        ...
        return self.ping_check(ip_address, count=count, v6=v6, fqdn_check=False, **kwargs)
    ...
    def connect(self, ssid=None, psk=None, ifname=None, bssid=None, key_mgmt=None, timeout=60,  # noqa: C901
                dhclient=True, e_gl_param='', e_net_param='', country='US', ipv4=True, ipv6=False,
                ipv6_stateless=False, wps=False, proto='RSN', pmf_mode='enabled', retry=3, eap=None,
                identity=None, password=None, **kwargs):  # noqa: C901
        """
...
"""
        ...
        with tempfile.NamedTemporaryFile(prefix=f"wpa_supp_conf_{self.config.get('user_name', '')}_", suffix='.conf',
                                         delete=True) as wpa_conf:
        ...
        command = f"sudo wpa_supplicant -D {driver} -i {ifname} -c {base_path}.conf -P {base_path}.pid " \
        ...
        if result[0]:
            ...
            return self._retry_connect_after_driver_crash(command, result,
                                                          ssid=ssid, psk=psk, ifname=ifname, bssid=bssid,
                                                          key_mgmt=key_mgmt, timeout=timeout, dhclient=dhclient,
                                                          e_gl_param=e_gl_param, e_net_param=e_net_param,
                                                          country=country, ipv4=ipv4, ipv6=ipv6,
                                                          ipv6_stateless=ipv6_stateless, wps=wps,
                                                          proto=proto, pmf_mode=pmf_mode, retry=retry, **kwargs)
    ...
    def _is_crash_in_txt(txt):
        return "brcmf_cfg80211_scan: scan error (-110)" in txt or "brcmf_cfg80211_scan: scan error (-52)" in txt or \
               ("Exception stack" in txt and "Workqueue" in txt and "Hardware name" in txt) \
    ...
    def start_dhcp_client(self, ifname, cf=None, ipv4=True, ipv6=False, ipv6_stateless=False,
                          timeout=20, clear_dhcp=True, **kwargs):
        """
        ...
        if ipv4:
            ...
            command = f"timeout {timeout} sudo dhclient -4 -pf /var/run/dhclient.{ifname}.pid {cf} " \
                      f"-lf /var/lib/dhcp/dhclient.{ifname}.leases " \
                      f"-I -df /var/lib/dhcp/dhclient6.{ifname}.leases {ifname}"
            start_time = time.time()
            ret = self.run_command(command, timeout=timeout + 20, **kwargs)
            if ret[0] and not ret[2]:
        ...
        if ipv6:
            self.run_command(f'rdisc6 {ifname}', **kwargs)
            log.info(f'Starting IPv6 dhclient for {ifname}')
            stateless = '-S ' if ipv6_stateless else ''
            # -df path is not an issue, it is according to the ifup
            command = f"timeout {timeout} sudo dhclient -6 {stateless} -pf /var/run/dhclient6.{ifname}.pid {cf} " \
                      f"-lf /var/lib/dhcp/dhclient6.{ifname}.leases " \
    ...
    def stop_dhcp_client(self, ifname, clear_cache=False, **kwargs):
        ...
        command = f"sudo ps aux | grep /var/run/dhclient.{ifname}.pid | grep -v grep | awk '{{print $2}}' | " \
            f"xargs sudo kill"
        self.run_command(command, **kwargs)
        command = f"sudo ps aux | grep /var/run/dhclient6.{ifname}.pid | grep -v grep | awk '{{print $2}}' | " \
            f"xargs sudo kill"
        self.run_command(command, **kwargs)
        if clear_cache:
    ...
    def disconnect(self, ifname=None, clear_dhcp=False, **kwargs):
        ...
        wpa_supp_process = self.run_command(f"ps aux | grep wpa_supplicant | grep {ifname} | grep -v grep"
                                            f" | awk '{{print $2}}'", **kwargs)
    ...
    def check(self, **kwargs):
        ...
        return self.run_command(f'{tool_path}/client-check', **kwargs)

    def pt_init(self, **kwargs):
        """Performance Tent Init"""
        tool_path = self.get_tool_path()
        return self.run_command(f"{tool_path}/pt-init", **kwargs)
    ...
    def start_continuous_flood_ping(self, ifname, file_path="/tmp/ping.log", target='8.8.8.8', **kwargs):
        ...
        cmd_output = self.run_command(f'/bin/ping -f -I {ifname} {target} > {file_path} &')
    ...
    def start_continuous_ping(self, ifname, file_path="/tmp/ping.log", wait='1', target='8.8.8.8', **kwargs):
        self.run_command("sudo killall ping", **kwargs)
        cmd_output = self.run_command(f"/bin/ping -I {ifname} -W {wait} {target} > {file_path} &")
        if not cmd_output[0] == 0:
            return False
        return True
    ...
    def create_ap(self, channel, ifname='', ssid='test', extra_param='', timeout=120, dhcp=False, country='US',
                  **kwargs):
        """
...
{extra_param}"""
        ...
        if dhcp:
            ...
            if "rpi_server" not in self.version(short=False, skip_ns=ns_out)[1]:
                ...
                res = self.run_command(
                    ...
                    f'; echo nameserver 8.8.8.8 | sudo tee /etc/resolv.conf"', skip_ns=ns_out)
                if res[0]:
                    return res
        ...
        command = f'sudo hostapd -d -f /tmp/hostapd_{ifname}.log -t -B -P /tmp/hostapd_{ifname}.pid' \
        ...
        overlapping_check = self.run_command(f'sudo cat /tmp/hostapd_{ifname}.log | grep "operation not permitted"',
                                             skip_ns=ns_out, **kwargs)
...
"""
        ...
        ret = self.run_command(f"sudo dhcpd -4 -q -cf /tmp/dhcpd_{ifname}.conf -pf /tmp/dhcpd_{ifname}.pid",
                               skip_ns=ns_out, **kwargs)
        response = self.merge_result(response, ret)
        return response
    ...
    def disable_ap(self, ifname='', **kwargs):
        ns_out = False
        ifname = ifname if ifname else self.get_wlan_iface()
        # probably iface was moved out of the network namespace so check there
        if not ifname:
            ifname = ifname if ifname else self.get_wlan_iface(force=True, skip_ns=True)
            ns_out = True

        if not ifname:
            return [1, "Cannot find any WiFi interface on the device", "Cannot find any WiFi interface on the device"]
        ...
        command = f"sudo ps aux | grep -P {dhcpd_path} | grep -v grep | awk '{{print $2}}' | " \
            f"xargs sudo kill"
    ...
    def refresh_ip_address(self, iface=None, ipv4=True, ipv6=False, ipv6_stateless=False, timeout=10, reuse=False,
                           static_ip=None, clear_dhcp=True, reuse_only=False, **kwargs):
        """
        Refresh ip_address.
        Args:
        ...
        return self.start_dhcp_client(iface, ipv4=ipv4, ipv6=ipv6, ipv6_stateless=ipv6_stateless, timeout=timeout,
                                      clear_dhcp=False)
    ...
    def start_selenium_server(self, port=4444, server_path='/usr/bin/selenium-server-standalone-3.141.59.jar',
                              **kwargs):
        response = self.run_command(f'export MOZ_HEADLESS=1; '
                                    f'java -jar {server_path} -port {port} &> /dev/null &',
                                    **kwargs)
        return response
    ...
    def start_simulate_client(self, device_to_simulate, ifname, ssid, psk, bssid, fake_mac):
        ...
        adt_lib.set_device(device_object=self, client_name=self.get_nickname(), interface_name=ifname,
                           default_mac=device_mac)
        ...
        if not device_cfg:
            return [5, '', f'Not found any device config for {device_to_simulate}.\n '
        ...
        return [0, f'{device_to_simulate} is simulated with following MAC address: {adt_lib.new_client_mac}\n'
    ...
    def hackrf_generate_radar_pulse(self, channel, region='us', vector=0, **kwargs):
        ...
        version = self.get_stdout(self.run_command(f"head -n2 {self.config[TBCFG_CLIENT_DEPLOY]}/radar",
                                                   **kwargs), skip_exception=True, **kwargs)
        if "version: 1.0.3" not in version:
            self.deploy()

        ret = self.run_command(f"python2 {self.config[TBCFG_CLIENT_DEPLOY]}/radar -c {channel} -r {region}"
                               f" -v {vector} pulse", timeout=35, **kwargs)
        return ret
    ...
    def get_ble_pair_token(self, serial_id, timeout=180, **kwargs):
        ...
        """
        script_output = self.run_command('sudo /tools/bt/plume_ble_config.py '
                                         f'--scan --connectable --token-change --sn={serial_id} --timeout={timeout}',
                                         timeout=timeout + 20, **kwargs)
        if script_output[0]:
    ...
    def send_cfg_via_ble(self, serial_id, ble_pin, cfg_data, timeout=180, **kwargs):
        ...
        """
        script_output = self.run_command(f"sudo /tools/bt/plume_ble_config.py --sn={serial_id} --passkey={ble_pin} "
                                         f"--timeout={timeout} -d '{cfg_data}'", timeout=timeout + 20, **kwargs)
        return script_output
    ...
    def recover_namespace_service(self, **kwargs):
        ...
        timeout = time.time() + 60
        while timeout > time.time():
            if self.get_stdout(self.uptime(timeout=10, skip_ns=False, **kwargs), skip_exception=True) and \
                    self.get_wlan_iface(force=True, skip_ns=False, **kwargs):
    ...
    def _is_node_support_6g(self):
        result = [device.get('name') for device in self.config['Nodes']
                  if device.get('capabilities', {}).get('interfaces', {}).get('home_ap', {}).get('6g')]
        return True if result else False

    def is_sae_key_mgmt(self, bssid, default_key_mgmt='WPA-PSK', scan_timeout=45):
        # Wi-Fi 6E supports only WPA3. Automatically set WPA3 mode if needed.
        # Check only when at least one device supports 6g and used client is Linux (rpi does not support WPA3)

        # TODO: instead of client type, use the capability checks (currently commented-out)
        # client_capab = self.get_wlan_information()[1]
        # client_wpa3_capab = client_capab['wlan'][self.wlan_iface].get('wpa3', False)
        if not self._is_node_support_6g() or self.type != 'linux':
            return default_key_mgmt
        sae_mode = False
        found_bssid = False
        timeout = time.time() + scan_timeout
        while timeout > time.time():
            scan_result = self.scan(params='| egrep "BSS |* Authentication"')
            # Skip checking key mgmt in case no scan results
            if scan_result[0]:
                continue
            for scan_entry in scan_result[1].split('BSS '):
                scan_entry = scan_entry.lower()
                if bssid not in scan_entry:
                    continue
                sae_mode = True if 'sae' in scan_entry else False
                found_bssid = True
                break
            if found_bssid:
                break
        return 'SAE' if sae_mode else default_key_mgmt
    ...
    def mocha_enable(self, ssid=None, psk=None, ifname=None, bssid=None, key_mgmt=None, **kwargs):
        ...
        crontab_cmd = r'crontab -l | { cat; echo "*/30 * * * * ' \
                      r'sudo ip netns exec nswifi1 ' \
                      r'wget -o /tmp/10_mb.txt http://192.168.7.1:8082/10_mb.txt; ' \
    ...
    def mocha_disable(self, ifname=None, **kwargs):
        ...
        download_and_rm_file_cmd = 'sudo ip netns exec nswifi1 ' \
                                   'wget -o /tmp/10_mb.txt http://192.168.7.1:8082/10_mb.txt; sudo rm /tmp/10_mb.txt'

        crontab_rm_cmd = f'crontab -l | grep -v "{download_and_rm_file_cmd}"  | crontab -'

        set_basic_wpa_supp_cfg = f'echo "{basic_wpa_supplicant_cfg}" | sudo tee /etc/wpa_supplicant/wpa_supplicant.conf'

        wlan_ifname = ifname if ifname else self.get_wlan_iface(**kwargs)

        outputs.append(self.run_command(crontab_rm_cmd, skip_ns=True))
    ...
    def get_client_model_from_dmidecode(self, **kwargs) -> str:
        ...
        try:
            manufacturer = re.sub(r'Manufacturer: ', '',
                                  re.search(r'Manufacturer: .*\n', dmi_output[1]).group(0)).strip()
    ...
    def get_wlan_model(self, **kwargs):
        ...
        if re.search(r'..:.... Network controller: ', lspci_output[1]):
            model = re.sub(r'..:.... Network controller: ', '',
                           re.search(r'..:.... Network controller: .*\n', lspci_output[1]).group(0)).strip()
        else:
            model = re.sub(r'..:.... PCI bridge: ', '',
                           re.search(r'..:.... PCI bridge: .*\n', lspci_output[1]).group(0)).strip()
...
class LinuxClientUpgrade:
    ...
    def download_image_from_url(self, http_address):
        ...
        image_name = [file_name for file_name in files_name if
                      re.search(r'.deb', file_name)]
    ...
    def _start_upgrade(self, fw_path, force, download_url, target_version, **kwargs):
        ...
        # add route over mgmt iface
        res = self.lib.run_command('sudo sh -c "ip r add default via 192.168.4.1  proto static  metric 100'
    ...
    def install_packages(self, *, packages, force, timeout, **kwargs):
        ...
        return self.lib.run_command(f'sudo DEBIAN_FRONTEND="noninteractive" apt-get -y'
                                    f' --allow-unauthenticated -o DPkg::Options::="--force-confnew"'
                                    f' install {reinstall} {packages}', timeout=timeout, **kwargs)
