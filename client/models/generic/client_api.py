...
class ClientApi(DeviceApi):
    ...
    def setup_class_handler(self):
        if self.session_config.option.skip_init or not self.lib.device or not self.lib.main_object:
            return
        ...
        if not uptime_result or not self.lib.check_wireless_client():
            ...
            if -1 < last_request_time < 120 and rpower_status == 'ON':
                ...
                while time.time() < timeout:
                    uptime_result = self.lib.get_stdout(self.lib.uptime(timeout=3, skip_logging=True),
                                                        skip_exception=True)
                    if uptime_result:
                        break
                    time.sleep(5)
        ...
        try:
            ...
            ver = "100.0.0"
        assert StrictVersion(ver) >= StrictVersion(self.min_required_version), f"FW on the client {self.lib.name} " \
    ...
    def teardown_class_handler(self):
        ...
        AllureUtil(self.session_config).add_allure_group_envs(f'client_{self.lib.name}', 'region', region,
                                                              f"file://{self.lib.name}")
        super().teardown_class_handler()
    ...
    def eth_connect(self, pod, port_alias=None, ifname=None, dhclient=True,
        ...
        result = self.lib.eth_connect(pod, port_alias=port_alias, ifname=ifname, dhclient=dhclient,
                                      ipv4=ipv4, ipv6=ipv6, ipv6_stateless=ipv6_stateless, **kwargs)
        return self.get_stdout(result, **kwargs)

    def eth_disconnect(self, ifname=None, **kwargs):
        """Disconnect client from all Ethernet pod ports."""
        result = self.lib.eth_disconnect(ifname, **kwargs)
        return self.get_stdout(result, **kwargs)

    def connect(self, ssid=None, psk=None, ifname=None, bssid=None, key_mgmt=None, timeout=60,
                dhclient=True, e_gl_param='', e_net_param='', country=None, ipv4=True, ipv6=False, ipv6_stateless=False,
                wps=False, retry=3, eap=None, identity=None, password=None, **kwargs):
        ...
        result = self.lib.connect(ssid, psk, ifname=ifname, bssid=bssid, key_mgmt=key_mgmt, timeout=timeout,
                                  ...
                                  ipv4=ipv4, ipv6=ipv6, ipv6_stateless=ipv6_stateless, wps=wps, retry=retry, eap=eap,
                                  identity=identity, password=password, **kwargs)
    ...
    def start_dhcp_client(self, ifname='', cf=None, ipv4=True, ipv6=False, ipv6_stateless=False,
        ...
        result = self.lib.start_dhcp_client(ifname, cf, ipv4=ipv4, ipv6=ipv6, ipv6_stateless=ipv6_stateless,
    ...
    def refresh_ip_address(self, iface=None, ipv4=True, ipv6=False, ipv6_stateless=False, timeout=10, reuse=False,
                           static_ip=None, clear_dhcp=True, **kwargs):
        response = self.lib.refresh_ip_address(iface, ipv4, ipv6, ipv6_stateless, timeout, reuse, static_ip, clear_dhcp,
                                               **kwargs)
    ...
    def get_stored_mac_address(self, iface='', **kwargs):
        ...
        client_mac = getattr(self.lib, f'{iface}_mac', None) if getattr(self.lib, f'{iface}_mac', None) else \
            self.get_mac(iface, **kwargs)
        return client_mac
    ...
    def remove_dir(self, path, **kwargs):
        assert ' ' not in path, f'Whitespace is not allowed in path'
