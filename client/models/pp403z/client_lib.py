...
class ClientLib(ClientLibGeneric):
    ...
    @staticmethod
    def get_ext_path():
        ext_path = ':'.join(['/bin:/sbin:/usr/bin:/usr/sbin:/opt/bin:/opt/sbin:/usr/opensync/tools:/usr/opensync/bin',
                             '/usr/opensync'])
        return ext_path
    ...
    def connect(self, ssid=None, psk=None, bssid=None, band='24g', key_mgmt='WPA-PSK', timeout=60,  # noqa: C901
                dhclient=True, e_gl_param='', e_net_param='', country='US', proto='RSN',  # noqa: C901
                pmf_mode='enabled', **kwargs):  # noqa: C901
        """
        ...
        wpa_supp_conf = self.create_wpa_conf(ssid=ssid, psk=psk, bssid=bssid, ifname=ifname, proto=proto,
                                             key_mgmt=key_mgmt, pmf_mode=pmf_mode, country=country,
        ...
        command = f"wpa_supplicant -i {ifname} -c {base_path}.conf -P {base_path}.pid " \
    ...
    def disconnect(self, ifname=None, **kwargs):
        ...
        wpa_supp_process = self.run_command(f"ps | grep wpa_supplicant | grep {ifname} | grep -v grep"
                                            f" | awk '{{print $1}}'", **kwargs)
    ...
    def start_dhcp_client(self, ifname, timeout=60, **kwargs):
        result = self.run_command(f'udhcpc -i {ifname} -p /var/run/udhcpc-{ifname}.pid '
                                  f'-s /usr/opensync/bin/udhcpc.sh -A 10', **kwargs, timeout=timeout)
...
class ClientIface(Iface):
    ...
    def get_interface_from_band(self, band):
        ...
        assert band in ClientIface.ALLOWED_BANDS, f'Provided not supported band: {band}. ' \
                                                  f'Allowed bands: {ClientIface.ALLOWED_BANDS}'
