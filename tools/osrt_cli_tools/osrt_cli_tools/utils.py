...
from lib_testbed.generic.util.logger import log, log_level  # noqa: F401
...
verbosity_option = click.option(
    ...
    "--verbosity",
    count=True,
    help="Tool output verbosity. The `-v` can be used multiple times to increase verbosity. Triple `-vvv` "
    "will result with maximum verbosity. One `-v` flag will result with producing a TRACE-level debug file. "
    "The log file name will be autogenerated, and placed in the temporary directory as determined by the interpreter. "
    "The `--debug` flag is ignored when used in combination with verbosity.",
)
...
def set_log_level(level: int):
    """Set automation console stream logger handler to desired level. Configure it if not configured already.
    This function intentionally doesn't change the log file levels.
    """
    from lib_testbed.generic.util.logger import log

    log.trace("Setting console log level to %s [%s]", logging.getLevelName(level), level)
    logger = logging.getLogger("automation")
    ...
    for handle in stream_handlers:
        handle.setLevel(level)


def set_log_verbosity(verbosity: int) -> None:
    """Set log handlers based on verbosity. Verbosity>0, then log file is created with TRACE-level logs.
    Increasing verbosity this function will set info, debug and trace levels for console log output.
    """
    ensure_console_handler()
    if not verbosity:
        set_log_level(log.WARNING)
    else:
        log.trace("Got user verbosity: %s", verbosity)
        log.ensure_file_handler()
        verbosity_levels = [log.INFO, log.DEBUG, log.TRACE]
        try:
            console_level = verbosity_levels[verbosity - 1]
        except IndexError:
            console_level = log.TRACE
        log.trace(
            "Established user desired console log level as %s [%s]", logging.getLevelName(console_level), console_level
        )
        log.set_log_levels(console_level=console_level, file_level=log.TRACE)
