...
class AttenuatorLib:
    ...
    def init_config(self, test_case):
        ...
        if not attenuators_config:
            raise OpenSyncException('Attenuator configuration not found.',
        ...
        for attenuator in attenuators_config:
            ...
            attenuator_obj = AttenuatorObject(ssh_gateway_cfg, attenuator['host'], attenuator['port'],
                                              attenuator['password'], test_case, name)
    ...
    def get_ssh_gateway_cfg(self, attenuator_cfg):
        ssh_gateway = attenuator_cfg.get('ssh_gateway') if attenuator_cfg.get('ssh_gateway') \
            else self.config.get('ssh_gateway')
        if not ssh_gateway:
    ...
    def get_attenuator_obj(self, att_indicator):
        ...
        for att_alias in att_aliases:
            ...
            if set(att_indicator) == set(att_list_alias) or att_alias[0] == att_indicator:
                ...
                break
        if not att_obj:
            raise Exception(f'Attenuator not found for {att_indicator} attenuator list. '
...
class AttenuatorObject:
    ...
    def execute_request(self, request, **kwargs):
        url = ATTENUATOR_URL.format(self.host, self.port, request) if not self.password else \
            ATTENUATOR_URL.format(self.host, self.port, self.password, request)
        output = self.ssh_gateway.run(f'curl {url}', **kwargs) if self.test_case \
