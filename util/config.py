...
FIXED_HOST_CLIENTS = [LOCAL_HOST_CLIENT_NAME, REMOTE_HOST_CLIENT_NAME, IPTV_HOST_CLIENT_NAME, S2S_VPN_HOST_CLIENT_NAME,
                      P2S_VPN_HOST_CLIENT_NAME, MOTION_HOST_CLIENT_NAME]
    ...
    for dir_name in dir_names:
        ...
        for file in list_dir:
            if re.compile(file_name).match(file):
                file = dir_name + '/' + file
                return file
    raise OpenSyncException(f'Cannot find config file {file_name}',
                            f'Expecting config file: {file_name} in {dir_names}')
...
def get_model_properties_dirs(device_type='node'):
    if device_type == 'node':
        return [os.path.join(BASE_DIR, CONFIG_DIR, MODEL_PROPERTIES_DIR, MODEL_INTERNAL_DIR),
                os.path.join(BASE_DIR, CONFIG_DIR, MODEL_PROPERTIES_DIR, MODEL_REFERENCE_DIR)]
    else:
        return [os.path.join(BASE_DIR, CONFIG_DIR, MODEL_PROPERTIES_DIR, MODEL_INTERNAL_DIR, CLIENT_DIR),
...
def get_misc_files():
    try:
        return [os.path.join(get_config_dir(), MISCS_DIR, misc_file)
                for misc_file in os.listdir(os.path.join(get_config_dir(), MISCS_DIR))]
...
def decrypt_ssh_key(config):
    def update_single_host(host_info):
        ...
        # use subprocess to hide system prints
        ret = subprocess.run(["ssh-keygen", "-p", "-P", ssh_key_passphrase, "-N", "", "-f", new_file],
                             capture_output=True)
        if ret.returncode:
...
def load_tb_config(location_file=None, deployment_file=None, sphere_file=None, skip_deployment=False,  # noqa:C901
    ...
    if not skip_deployment:
        if deployment_file and not os.path.isabs(deployment_file) and location_file:
            if config.get(TBCFG_PROFILE) != 'auto' and config.get(TBCFG_PROFILE) != deployment_file:
                log.info(f'Can not move location to "{deployment_file}" deployment because '
                ...
                if loc_deployment != deployment_file:
                    try:
                        ...
                        raise OpenSyncException("Required PlumeToolLib is missing", "Add cloud module")
                    ...
                    if plumelib.move_to_deployment(deployment_file):
...
def init_fixed_host_clients(tb_config):
    ...
    if not next(filter(lambda client_cfg: client_cfg['name'] == LOCAL_HOST_CLIENT_NAME, tb_config['Clients']),
                None):
        tb_config['Clients'].append(_generate_host_config(ssh_gateway, LOCAL_HOST_CLIENT_NAME))

    if not next(filter(lambda client_cfg: client_cfg['name'] == IPTV_HOST_CLIENT_NAME, tb_config['Clients']),
                None):
    ...
    if not any(client for client in tb_config['Clients'] if client.get('name') == S2S_VPN_HOST_CLIENT_NAME):
        tb_config['Clients'].append(_generate_host_config(ssh_gateway, S2S_VPN_HOST_CLIENT_NAME, 's2s-vpn'))

    if not any(client for client in tb_config['Clients'] if client.get('name') == P2S_VPN_HOST_CLIENT_NAME):
        tb_config['Clients'].append(_generate_host_config(ssh_gateway, P2S_VPN_HOST_CLIENT_NAME, 'rw-vpn'))
    ...
    if not next(filter(lambda client_cfg: client_cfg['name'] == REMOTE_HOST_CLIENT_NAME, tb_config['Clients']),
                None):
...
def _generate_host_config(ssh_gateway_config, client_name, namespace_name=None):
...
def get_location_name(config):
    if location_file := config.get('location_file'):
    ...
    for band, channels in capab['interfaces']['radio_channels'].items():
        ...
        for channel in channels[:]:
            if is_channel_dfs(channel):
                if not warn_printed:
                    warn_printed = True
                    log.warning(f"Removing DFS channels from supported radio channels for {capab['model_string']},"
...
def update_config_with_admin_creds(config):
    ...
    if (dpl := config.get('deployment_id', '')) in ['beta', 'chi', 'chi-staging', 'delta', 'gamma', 'iota', 'kappa',
                                                    'tau-int', 'tau-prod', 'theta']:
