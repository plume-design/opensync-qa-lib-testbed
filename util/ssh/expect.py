...
class ExpectHostInfo(object):
    def __init__(self, host, user, port, chained_ssh, expect, sshpass='', opts='', **kwargs):
        ...
        if self.sshpass:
            ...
            if 'shell_passwd' in expect:
                self.expect = "expect -c 'set timeout %d; spawn ssh -o StrictHostKeyChecking=no" \
                              " -o UserKnownHostsFile=/dev/null %s %s@%s %s; expect \"?password\" {send \"%s\\n\"; " \
                              "expect \"%s\" {send \"%s\\n\"; expect \"%s\"" \
                              " {send \"%s\\n\"; expect \"%s\";" \
                              " send {%s; %s; %s}; send \"\\n\"; expect \"%s\"; %s; %s}}}; expect timeout {exit 255}'" \
                              % (expect['timeout'], self.opts, self.user, self.host, self.port, self.sshpass,
                                 expect['user_prompt'], expect['shell_cmd'], expect['shell_passwd_prompt'],
                                 expect['shell_passwd'], expect['shell_prompt'], '%s', '%s', '%s',
                                 expect['shell_prompt'], expect['shell_exit'], expect['user_quit'])
            else:
                ...
                self.expect = "expect -c 'set timeout %d; spawn ssh -o StrictHostKeyChecking=no" \
                              ...
                              " send {%s; %s; %s}; send \"\\n\"; expect \"%s\"; %s; %s}}; expect timeout {exit 255}'"\
                              % (expect['timeout'], self.opts, self.user, self.host, self.port, self.sshpass,
                                 expect['user_prompt'], expect['shell_cmd'], expect['shell_prompt'], '%s', '%s', '%s',
                                 expect['shell_prompt'], expect['shell_exit'], expect['user_quit'])
        else:
            self.expect = "expect -c 'set timeout %d; spawn ssh -o StrictHostKeyChecking=no" \
                          ...
                          % (expect['timeout'], self.opts, self.user, self.host, self.port, expect['user_prompt'],
                             expect['shell_cmd'], expect['shell_prompt'], '%s', '%s', '%s', expect['shell_prompt'],
                             expect['shell_exit'], expect['user_quit'])
    ...
    def command_wrapper(self, command):
        if command:
            ...
            exp_cmd = f"{sshpass} ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -o LogLevel=quiet" \
