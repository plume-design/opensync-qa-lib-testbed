...
class DeviceLogCatcher(LogCatcher):
    ...
    def collect(self, test_data):
        ...
        system_info += "Device_log_catcher: 2.0.7\n"
        ...
        if device_type == "pod":
            ...
            log_pull_gzip = self.obj.get_stdout(
                self.obj.get_logs(log_pull_dir, timeout=120, skip_logging=True), skip_exception=True
            )
            ...
            dmesg = (
                self.obj.get_stdout(
                    self.obj.run_command("dmesg -T", skip_logging=True, timeout=60), skip_exception=True
                ),
                "dmesg",
            )
            syslog = (
                self.obj.get_stdout(
                    self.obj.run_command("cat /var/log/syslog | tail -n 2000", skip_logging=True, timeout=60),
                    skip_exception=True,
                ),
                "syslog",
            )
            ...
            wpa_supplicant_log = (
                self.obj.get_wpa_supplicant_file(lines=10000, extension="log", timeout=60, skip_logging=True),
                "wpa_supplicant_log",
            )
            wpa_supplicant_conf = (
            ...
            if iface:
                dhcp_v4_leases = (
                    self.obj.get_stdout(
                        self.obj.run_command("cat /var/lib/dhcp/dhclient.wlan0.leases", skip_logging=True, timeout=60),
                        skip_exception=True,
                    ),
                    "dhcp_v4_leases",
                )
                dhcp_v6_leases = (
                    self.obj.get_stdout(
                        self.obj.run_command("cat /var/lib/dhcp/dhclient6.wlan0.leases", skip_logging=True, timeout=60),
                        skip_exception=True,
                    ),
                    "dhcp_v6_leases",
                )
            else:
                dhcp_v4_leases, dhcp_v6_leases = ("", ""), ("", "")
            for command_dump in [
                dmesg,
                syslog,
                iwconfig,
        ...
        self.initial_logger(
            name=f"log_pull_{device_type}_{dev_name}",
            mime_type="application/gzip",
            source=log_pull_zip,
            dirs_to_remove=(log_pull_dir,),
        )
    ...
    #
    def make_archive(
        self, base_name, format, root_dir=None, base_dir=None, verbose=0, dry_run=0, owner=None, group=None, logger=None
    ):
        save_cwd = os.getcwd()
    ...
    def make_zipfile(self, base_name, base_dir, verbose=0, dry_run=0, logger=None):
        ...
        if not dry_run:
            with zipfile.ZipFile(zip_filename, "w", compression=zipfile.ZIP_DEFLATED, strict_timestamps=False) as zf:
