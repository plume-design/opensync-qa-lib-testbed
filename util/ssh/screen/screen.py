...
class ScreenHostInfo(object):
    ...
    def command_wrapper(self, command, timeout=20, expect=None, end_sign=True, stuff=False):
        ...
        if not stuff and "^M" not in command:
            if 1:
                ...
                detach = "-d"
            screen_cmd = (
                f"sudo expect -c 'spawn screen -x {detach} {self.screen} ; "
    ...
    def prepare_command(self, command):
        if "/etc/init.d/manager" in command:
            command = command.replace("/etc/init.d/manager", "/etc/init.d/managers")
        elif "ovsdb-client dump" in command:
            command = command.replace(
                "ovsdb-client dump", "LD_LIBRARY_PATH=/usr/plume/lib; ovsdb-client dump unix:/var/run/db.sock"
            )
        elif "lm_logs_collector" in command:
            command = command.replace("bin/lm_logs_collector.sh", "tools/lm_log_pull.sh")
        # do not take system PATH, as then there is a problem with extracting the output from the console
        if "$PATH" in command:
...
def parse_screen_output(cmd, stdout):  # noqa: C901
    expect = ""
    end_sign = ""
    ...
    if len(expect_list) > 1:
        ...
        if "end_sign" in cmd:
            end_sign = re.search(r"end_sign\d+", expect, re.MULTILINE).group()
        expect = expect[3:]
    ...
    if expect and not end_sign:
        ...
        if len(stdout_join.split(end_sign)) > 4:
            stdout_tmp = "\n".join("".join(stdout_tmp.split(end_sign)[2:-2]).splitlines()[1:-1])
        else:
            stdout_tmp = "\n".join("".join(stdout_tmp.split(end_sign)[-2:-1]).splitlines()[1:-1])
    for line in stdout_tmp.splitlines():
        if "sc nodebug 0" in line or line.startswith("#") or "~#" in line:
            continue
        # sometimes DUT echoes so we need to skip the command
        if line in cmd:
            continue
