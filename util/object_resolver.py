...
LIB_TESTBED_DIR = Path(__file__).absolute().parents[2].as_posix()
...
class ObjectResolver:
    ...
    def resolve_pod_class(file_name, model, wifi_vendor=None):
        pod_path_file = ObjectResolver.resolve_model_path_file(
            file_name=file_name, model=model, wifi_vendor=wifi_vendor
        )
