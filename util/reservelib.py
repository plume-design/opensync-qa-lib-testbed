...
class ReserveLib:
    ...
    def _get_crnt_reservation_msg(self):
        ...
        message = ''
        try:
            file_operation_cmds = f'flock -x "$reservefd"; ' \
                                  f'tail -1 {self.res_file}; ' \
                                  f'sync; '
    ...
    def reserve_test_bed(self, timeout=120):
        ...
        if not self.check_reservation_possible():
            return {'name': self.tb_name, 'status': False, 'owner': 'CANNOT RESERVE THIS TB',
                    'since': "-", 'expiration': "-"}
        ...
        if not self.tb_config.get('force'):
            ...
            if status['busy'] and not status['busyByMe']:
                log.error(f"{self.tb_name} is used by {status['owner']}."
                          f" Expires at: {status['expiration']}")
                return {'name': self.tb_name, 'status': False, 'owner': status['owner'],
                        'since': f"{status['since']}", 'expiration': f"{status['expiration']}"}
        ...
        file_operation_cmds = f'flock -x "$reservefd"; ' \
                              f'tail -2000 {self.res_file} | tee {self.res_file}; ' \
                              f'sync; '
        cmd = self.openfd_cmd + file_operation_cmds + self.closefd_cmd
        ...
        if out.startswith('Error:'):
            return {'name': self.tb_name, 'status': False, 'owner': "ERROR: CANNOT RESERVE",
                    "since": status['since'], 'expiration': "-"}
        ...
        rsrv_dict = {'name': self.tb_name, 'status': True, 'owner': res_row[0],
    ...
    def get_reservation_status(self, tool=True):
        ...
        """
        if not self.check_reservation_possible():
            return {'name': self.tb_name, "busy": False, "busyByMe": False, "owner": "CANNOT GET RESERVATION",
                    "since": "-", "expiration": "-"}
        file_operation_cmds = f'flock -x "$reservefd"; ' \
                              f'tail -1 {self.res_file}; '
        ...
        if not out:
            file_operation_cmds = f'flock -x "$reservefd"; ' \
                                  f'[ -s {self.res_file} ] || echo empty file; '
            cmd = self.openfd_cmd + file_operation_cmds + self.closefd_cmd
        ...
        if "No such file or directory" in out or 'cannot open lock file' in out or 'empty file' in out:
            return {'name': self.tb_name, "busy": False, "busyByMe": False, "owner": "-",
                    "since": "-", "expiration": "-"}
        ...
        if busy:
            return {'name': self.tb_name, "busy": busy, "busyByMe": busy_by_me, "owner": info[0],
                    "since": f"{info[2]}", "expiration": f"{info[3]}"}
        else:
            info[3] = f"Expired: {info[3]}" if tool else '-'
            return {'name': self.tb_name, "busy": busy, "busyByMe": busy_by_me, "owner": f"Last owner: '{info[0]}'",
                    "since": "-", "expiration": info[3]}
    ...
    def unreserve(self):
        ...
        if status['owner'] == 'CANNOT GET RESERVATION':
            return {'name': self.tb_name, "status": False, "owner": "ERROR: CANNOT GET RESERVATION", 'since': "-",
                    "expiration": "-"}
        ...
        if out.startswith('Error:'):
            return {'name': self.tb_name, 'status': False, 'owner': "ERROR: CANNOT UNRESERVE",
                    "since": status['since'], 'expiration': "-"}
        res_row = res_row.split('__')
        return {'name': self.tb_name, 'status': True, 'owner': "-",
                'since': f"{self._convert_utc_to_local(res_row[2])}", 'expiration': "-"}
    ...
    def clear_reservation_history(self):
        ...
        if status['busy']:
            file_operation_cmds = f'flock -x "$reservefd"; ' \
                                  f'echo "$(tail -1 {self.res_file})" | sudo tee {self.res_file}; ' \
                                  f'sync;'
            cmd = self.openfd_cmd + file_operation_cmds + self.closefd_cmd
    ...
    def usage_statistics(self, time_res='week', min_use='0'):
        """
        ...
        for line in out.split('\n')[::-1]:
            ...
            if len(reservation) < 4:
                if line:  # report it, unless it's just an empty line
                    log.warn(f'Invalid reservation line for {self.tb_name}: "{line}"')
                continue
    ...
    def generate_reservation_row(self, current_reservation, reservation_time=120, keep_owner=False):
        ...
        if end_time == '-':
            ...
            if not (reservation_time in [10, 3]
                    and datetime.datetime.utcnow() + datetime.timedelta(minutes=10) < end_time):
                end_time = datetime.datetime.utcnow() + datetime.timedelta(minutes=reservation_time)
...
class PyReservePlugin:
    ...
    def _reserve(self):
        ...
        while time.time() < timeout:
            ...
            if status['owner'] == "CANNOT GET RESERVATION":
                err_cnt += 1
                if err_cnt < 3:
                    log.warning("Cannot check reservation, trying again in 3 sec...")
                    time.sleep(3)
                    continue
                log.warning(f'End up with checking reservation for {status.get("name")}, '
            ...
            assert False, f"Testbed still reserved!"
    ...
    def _unreserve(self):
        ...
        if self.reserve_t and self.reserve_t.is_alive():
            ...
            self.reserve_t.join()
            log.info(f'Reservation thread -> "{self.reserve_t.getName()}" current state == '
...
class ReserveLibLogCatcher(LogCatcher):
    ...
    def ssh_caller(self, remote_host, caller_name, command, output):
        ...
        if command.startswith('echo "$(tail -2000') and '\n' in output:
            ...
            output_dict.update({
                'remote_output': truncated_out,
                'truncated': True
            })
        self.add({'function_name': caller_name,
