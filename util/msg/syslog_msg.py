...
class SyslogMsg:
    ...
    def cmd(self, process=None, last_lines=1000, cursor=None, timeout=30, module=None):
        ...
        for line in self.pod.get_stdout(result, skip_exception=True).splitlines():
            ...
            msg_cursor = message["timestamp"] + cursor_id / 100.0
            if cursor and msg_cursor <= cursor:
                continue
            message["cursor"] = msg_cursor
            messages.append(message)

        if last_lines and last_lines < len(messages):
            messages = messages[-last_lines:]
        return messages
