...
class Msg:
    ...
    def get_service_log(self, *, process=None, cursor=None, timeout=30, module=None, regex=".*"):
        return self._cmd_wait(process=process, cursor=cursor, timeout=timeout, module=module, regex=regex)

    def get_logs_to_print(
        self, *, process=None, cursor=None, timeout=30, module=None, regex=".*", service_logs: list = None
    ):
        if not service_logs:
            service_logs = self.get_service_log(
                process=process, cursor=cursor, timeout=timeout, module=module, regex=regex
            )
        service_logs = [
            f'{log_row["date_timestamp"]} <{process}> <{log_row["severity"]}> <{log_row["module"]}> '
            f'{log_row["value"]}'
            for log_row in service_logs
        ]
        return "\n".join(service_logs)
