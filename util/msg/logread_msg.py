...
class LogreadMsg:
    ...
    def cmd(self, process=None, last_lines=1000, cursor=None, timeout=30, module=None):
        ...
        out = self.pod.run_command(logread_cmd, timeout=timeout, retry=False)
    ...
    def parse_logread_resp(self, resp, last_lines, from_cursor):
        ...
        for line in resp:
            try:
                # Set strict to False because sometimes the log entry can not be parsed due to control characters.
                line_info = json.loads(line, strict=False)
            except ValueError:
        ...
        for message in messages:
            ...
            if from_cursor:
                if cursor <= from_cursor:
                    continue
            message_info = {
                "date_timestamp": message["message_tm"],
                "timestamp": timestamp,
                "value": value,
                "cursor": cursor,
