...
def get_fw(models, version="latest") -> tuple[str, int]:
    """
    Args:
...
def get_ipv6_capable_and_downloadable_fw(models) -> tuple[str, int]:
    """Returns ipv6 capable firmware. In case none is available, falls back
    to "first" firmware."""
    test_fw = get_fw(models, "ipv6")
    if test_fw[0]:
        return test_fw
    return get_fw(models, "first")
