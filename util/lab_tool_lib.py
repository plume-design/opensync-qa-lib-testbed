...
from lib_testbed.generic.util.ssh.ssh_execute import SshExecute
...
class LabToolLib:
    ...
    def upgrade_clients(self, version="latest"):
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            future_upgrade_client = {
                executor.submit(self._single_test_bed_clients_upgrade, tb_name, version): tb_name
    ...
    def versions(self):
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            future_version = {
                executor.submit(self._single_test_bed_versions, tb_name): tb_name for tb_name in self.tb_configs
            }
    ...
    def fuse_status(self):
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            future_status = {
                executor.submit(self._single_test_bed_fuse_status, tb_name): tb_name for tb_name in self.tb_configs
            }
    ...
    def _single_test_bed_clients_upgrade(self, tb_name, version):
        ...
        print(f"[{tb_name}] Upgrading server")
        server_lib = self._get_client_lib(tb_config, "server")
        ...
        # upgrade clients
        client_lib = self._get_client_lib(tb_config, "clients")
        print(f"[{tb_name}] Upgrading clients")
        try:
        ...
        for nickname, ret_client in zip(client_lib.get_nicknames(), ret):
            ...
            if ret_client[2]:
                std_err += f"\n\n{nickname}:\n{ret_client[2]}"
        reserve_tb.un_reserve_testbed()
        print(f"[{tb_name}] Upgrade finished for all clients")
        print(f"[{tb_name}] Updating switch with latest config")
        switch_api, switch_controller = self._get_switch_lib(tb_config)
        ret = switch_controller.restore_config()
        result = SshExecute.merge_result([err_code, std_out, std_err], ret)
        all_ports = switch_api.get_list_of_all_port_names()
        switch_api.recovery_switch_configuration(all_ports)
        print(f"[{tb_name}] Switch config updated and configuration restored")
        return result
    ...
    @staticmethod
    def _get_switch_lib(tb_config):
        try:
            switch_api = SwitchApiResolver(config=tb_config)
            switch_controller = SwitchController(tb_config=tb_config, switch_api=True)
            return switch_api, switch_controller
        except Exception:
            traceback.print_exc(limit=2, file=sys.stdout)
            raise
