...
class SanityLib(object):
    def __init__(self, tables, gw_tables, logs_dir=None, outfile=None, outstyle='full', capabilities=None):
        ...
        self.backhaul_ap, self.backhaul_sta, self.lan_interfaces, self.regulatory_domains = [], [], [], []
        ...
        for row in table:
            ...
            if val == hsub:
                ...
                if not self.check_wds:
                    if not gre:
                        self._create_output(tname, 'ERROR', str(row['inet_addr'])
                                            + ' is missing downlink GRE interface'
                                            ' in Wifi_Inet_Config')
                        errs = errs + 1
                    else:
                        prt = ovsdb.ovsdb_find_row(self.tables['Port table'], 'name', gre['if_name'])
                        if not prt:
                            self._create_output(tname, 'ERROR', str(gre['if_name'])
                                                + ' is missing from the Port table')
                        else:
    ...
    def _ovsdb_sanity_check_wifi_master_state_table(self):  # noqa
        ...
        lan_bridge_ifaces = [f"{self.lan_bridge}.{iface}" for iface in ['arp', 'dhcp', 'dns', 'dpi', 'http', 'l2uf',
                                                                        'ndp', 'tx', 'upnp']]
    ...
    def check_regulatory_domains(self):
        ...
        for table_column in wifi_radio_state_table:
            ...
            if not country_code:
                self._create_output(table_name, 'Warning',
                                    f'{band_name} Can not check country code - missing info from {table_name}')
                continue
            ...
            if self.regulatory_domains and country_code not in self.regulatory_domains[0]:
                self._create_output(table_name, 'ERROR',
        ...
        if not self.gw_node and self.gw_tables and self.gw_tables.get(table_name):
            gw_bands = [band_name['freq_band'] for band_name in
                        self.gw_tables.get(table_name) if band_name.get('freq_band')]
            ...
            for gw_band in gw_bands:
                ...
                if gw_country_code:
                    ...
                    gw_country_code = 'GB' if gw_country_code == 'UK' or gw_country_code == 'EU' \
                                              and 'GB' in set(self.regulatory_domains) else gw_country_code
    ...
    def get_country_code(wifi_radio_state_table, band_name):
        ...
        if not table_column:
            return None
        country_code = table_column.get('country') if table_column.get('country') \
    ...
    def _ovsdb_sanity_check_wifi_vif_config_table(self):  # noqa
        ...
        if home_ap_cnt != home_ap_list_cnt:
            self._create_output(state_tname, 'ERROR',
                                f'Number of home-ap {home_ap_cnt} != {home_ap_list_cnt}')
            errs = errs + 1

        bhaul_ap_list_cnt = len(bhaul_ap_list)
        if 'bhaul-ap-24' in req_list and bhaul_ap_cnt != bhaul_ap_list_cnt:
            self._create_output(state_tname, 'ERROR',
                                f'Number of bhaul-ap {bhaul_ap_cnt} != {bhaul_ap_list_cnt}')
            errs = errs + 1
        if self.gw_node:
            if bhaul_sta_cnt != 0:
                self._create_output(state_tname, 'ERROR',
                                    f'Number of bhaul-ap {bhaul_ap_cnt} != 0 on gateway')
                errs = errs + 1
        ...
        for crow in state_table:
            ...
            if not srow:
                self._create_output(state_tname, 'ERROR',
                                    f"Channel: {crow['channel']} not found in Wifi_Radio_State table")
                errs = errs + 1
    ...
    def _ovsdb_sanity_check_wifi_vif_config_table_bcm(self):  # noqa: C901
        ...
        if self.gw_node:
            if bhaul_sta_cnt != 0:
                self._create_output(state_tname, 'ERROR',
                                    f'Number of bhaul-stap {bhaul_sta_cnt} != 0 on gateway')
                errs = errs + 1
        elif bhaul_sta_cnt > 1:
    ...
    def ovsdb_sanity_check(self, **kwargs):
        ...
        reduced_mode_skip = ['_ovsdb_sanity_check_dhcp_leased_ip_table',
                             '_ovsdb_sanity_check_wifi_associate_clients_table',
                             '_ovsdb_sanity_check_wifi_inet_config_table']
        if self.device_mode != 'cloud':
            kwargs.update({'createOutput': {'tableName': 'AWLAN_Node table', 'logLevel': 'INFO',
        ...
        for r in req_list:
            ...
            if crow['ssid'] != srow['ssid']:
                self._create_output(config_tname, 'ERROR',
                                    f"ssid does not match: Config {crow['ssid']} State [{srow['ssid']}]")
                errs = errs + 1
            # check security
            ...
            if crow['mac_list_type'] is None or crow['mac_list_type'] == '':
                ...
                self._create_output(config_tname, 'ERROR', f"mac_list_type does not match: {crow['mac_list_type']} "
                                                           f"!= {srow['mac_list_type']}")
                errs = errs + 1
            # check mode
    ...
    def check_network_mode(self):
        ...
        if not self.wano:
            ...
            if prt_w2h and prt_h2w:
                ...
                if br_w2h and br_w2h['name'] == self.wan_bridge and br_h2w \
                        and br_h2w['name'] == self.lan_bridge:
                    router_mode = False
                else:
                    self.retval = False
        else:
    ...
    def _check_kernel_crash(self):
        ...
        self._create_output('KERNEL', lvl,
                            f"Uptime {up_time} sec [{up_time_str}], Boot Count={boot_cnt}{extra}")
    ...
    def _check_app_crash(self):
        ...
        if app_crashes or core_dumps or tar_app_crashes:
            self._create_output('USERSPACE', 'Warning', f'#### There are {app_crashes} app crashes, '
    ...
    def _check_vap(self):
        ...
        for x in vaps:
            state, msg = getstatusout(f"cat {self.logs_dir}iwconfig | grep -A 1 {x}[^0-9] |"
                                      " egrep 'Not-Associated' > /dev/null 2>&1")
            if not state:
    ...
    def _check_eth(self):
        ...
        if up_cnt or down_cnt:
            ...
            up_10 = int(getout(
                f"cat {self.sys_log_file}* | egrep -i '{self.wan_interfaces}: link up' | egrep '10Mbps' | wc -l"))
            self._create_output(f'{self.wan_interfaces} STATS', lvl,
                f'DOWN={down_cnt} times, UP={up_cnt} times ({up_1000}=1000Mbps, {up_100}=100Mbps, {up_10}=10Mbps)')

    def _check_cm(self):
        disconn_cnt = int(
            getout(f"egrep 'OVS connection changed from TRUE to FALSE' {self.sys_log_file}* | wc -l"))
        conn_cnt = int(
            getout(f"egrep 'OVS connection changed from FALSE to TRUE' {self.sys_log_file}* | wc -l"))
        ...
        self._create_output('CM STATS', lvl,
    ...
    def _check_wm(self):
        ...
        for sta in self.backhaul_sta:
            ...
            disassoc[sta] = int(
                getout(f"egrep '{sta}: STA still disassociated' {self.sys_log_file}* | wc -l"))
        ...
        self._create_output('WM STATS', total_lvl,
                            sfmt.format('Totals:', f'CSA={total_csa}', f'P.Change={total_pchange}',
                                        f'Conf={total_conf}', f'Assoc={total_assoc}',
                                        f'Disc={total_disc}', f'C.Conn={total_ap_assoc}',
                                        f'C.Disc={total_ap_disassoc}'))
        radio = {0: '2.4G', 1: '5G', 2: '5GU'}
        for i in range(len(self.backhaul_sta)):
            lvl = 'Warning' if disassoc[self.backhaul_sta[i]] > 5 or assoc[self.backhaul_sta[i]] - configuring[
                self.backhaul_sta[i]] > 5 else 'INFO'
            self._create_output('', lvl, sfmt.format(f'{radio[i]}:',
    ...
    def _check_dhcp_idx(self):
        ...
        if dup_ids:
            self._create_output('DHCP Idx', 'ERROR',
                                f'DHCP pool index collision in {link}, duplicate Id(s): {dup_ids}')
    ...
    def _check_single_dhcp_server(self):
        ...
        try:
            ...
            dhcpc = getout(
                f"cat {self.sys_log_file}* | egrep -i 'udhcpc.user: gateway=' | tail -n +3 | head -1")
            try:
