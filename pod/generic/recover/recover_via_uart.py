...
class Recover:
    ...
    def replace_dropbears(self):
        if "No such file or directory" in self.serial.cmd(f'ls /tmp/{self.dropbear_binary}'):
            log.warn(f'Trying to copy {self.dropbear_binary} once again')
            self.copy_dropbearmulti()
        assert 'No such file or directory' not in self.serial.cmd(f'ls /tmp/{self.dropbear_binary}'), \
    ...
    def serial_init(self):
        ...
        while True:
            ...
            if run_time_min < self.post_boot_delay / 60:
                if not init_done:
                    log.info("Current run time: {} min, wait {} min..".format(
