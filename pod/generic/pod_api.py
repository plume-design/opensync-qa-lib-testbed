...
from lib_testbed.generic.util.common import get_git_revision, mark_failed_recovery_attempt, get_target_pytest_mark
...
class PodApi(DeviceApi):
    ...
    def setup_class_handler(self):
        ...
        self.setup_class_handler_main_object()

    def setup_method_handler(self, method):
        super().setup_method_handler(method)

    def setup_class_handler_main_object(self):
        ...
        if hasattr(self, "session_config"):
            ...
            AllureUtil(self.session_config).add_allure_group_envs(f'node_{self.serial}', 'version', version,
                                                                  f"file://{self.serial}", fixed_value=True)
            AllureUtil(self.session_config).add_allure_group_envs(f'node_{self.serial}', 'model', self.model,
                                                                  f"file://{self.serial}", fixed_value=True)
            if region:
                AllureUtil(self.session_config).add_allure_group_envs(f'node_{self.serial}', 'region', region,
                                                                      f"file://{self.serial}")
            git_ver = get_git_revision()
            if git_ver:
                AllureUtil(self.session_config).add_environment('git_sha', git_ver, '_error')
    ...
    def teardown_class_handler(self):
        if not self.lib.device or (hasattr(self, "session_config") and self.session_config.option.skip_init):
            ...
            return
        # Perform actions for device which is marked as main object
        self.teardown_class_handler_main_object()

    def teardown_class_handler_main_object(self):
        if not self.lib.main_object:
            super().teardown_class_handler()
            return
        ...
        if self.lib.device.config['capabilities']['device_type'] == 'residential_gateway':
            ...
            return
        if hasattr(self, "session_config"):
            exp_ver = AllureUtil(self.session_config).get_allure_group_env(f'node_{self.serial}', 'version')
        else:
            exp_ver = None
        try:
            if cur_version and exp_ver and cur_version != exp_ver:
                log.warning(f"Test ended with incorrect FW version: {cur_version}, expected version: {exp_ver}.\n"
    ...
    def set_wano_configuration(self):
        if not hasattr(self, 'pytestmark') or self.obj_name.get('name', '') != 'gw':
            return
        wan_connection = get_target_pytest_mark(self.pytestmark, 'wan_connection')
    ...
    def kill_manager(self, wait_for_restart=False, **kwargs):
        ...
        response = self.lib.kill_manager(wait_for_restart, **kwargs)
        return self.get_stdout(response, **kwargs)
    ...
    def upgrade_fw_through_ssh(self, fw_matrix_obj, force=False, reboot_wait=True, **kwargs):
        ...
        request.urlretrieve(fw_url, f"/tmp/automation/{fw_file_name}")
    ...
    def start_wifi_blast(self, plan_id=None, blast_duration=None, blast_packet_size=None, blast_sample_count=None,
                         step_id_and_dest: list = None, threshold_cpu=None, threshold_mem=None, **kwargs):
        """ Start WiFi Blast through ovsdb api  """
        return self.lib.start_wifi_blast(plan_id=plan_id, blast_duration=blast_duration,
                                         blast_packet_size=blast_packet_size, blast_sample_count=blast_sample_count,
                                         step_id_and_dest=step_id_and_dest, threshold_cpu=threshold_cpu,
                                         threshold_mem=threshold_mem, **kwargs)
    ...
    def get_connection_flow_marks(self, ip_address, **kwargs):
        ...
        return self.lib.get_connection_flow_marks(ip_address=ip_address, **kwargs)
    ...
    def check_traffic_acceleration(self, ip_address, expected_protocol=6, multicast=False, flow_count=1, **kwargs):
        ...
        """
        return self.lib.check_traffic_acceleration(ip_address=ip_address, expected_protocol=expected_protocol,
                                                   multicast=multicast, flow_count=flow_count, **kwargs)

    def check_traffic_acceleration_flex(self, ip_address, wds=False, **kwargs):
        """Check traffic acceleration"""
        return self.lib.check_traffic_acceleration_flex(ip_address=ip_address, wds=wds, **kwargs)

    def get_connection_flows_dump(self, ip_address='', **kwargs):
        """Get connection flows dump for provided IP address"""
        return self.lib.get_connection_flows_dump(ip_address=ip_address, **kwargs)
