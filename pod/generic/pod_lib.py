...
DFS_REGION_MAP = {'EU': '0x0037', 'US': '0x003a', 'JP': '0x8FAF', 'CA': '0x0014', 'UK': '0x833A',
                  'NZ': '0x822A', 'SG': '0x82BE', 'IL': '0x8178', 'HK': '0x8158', 'KR': '0x005f',
                  'PH': '0x8260'}
...
class PodLib(PodBase):
    ...
    def get_opensync_path(self, **kwargs):
        ...
        res = self.get_stdout(self.run_command('ps | grep "bin/dm" | grep -v grep | head -n 1', **kwargs),
                              skip_exception=True)
        if not res:
    ...
    def reboot(self, **kwargs):
        """Reboot node(s)"""
        return self.run_command('reboot', **kwargs)
    ...
    def get_file(self, remote_file, location, **kwargs):
        ...
        if not os.path.isdir(location):
            try:
                ...
                return f'Could not create dir {location}: {e}.'
    ...
    def deploy(self, **kwargs):
        ...
        deploy_dir = ObjectResolver.resolve_model_path_file(
            file_name='deploy', model=self.device.config['model'],
    ...
    def check(self, **kwargs):
        ...
        node_check = ObjectResolver.resolve_model_path_file(
            file_name='deploy', model=self.device.config['model'],
            wifi_vendor=self.device.config['capabilities']['wifi_vendor']) + '/node-check'
    ...
    def upgrade_from_artifactory(self, image: str, *args, **kwargs):
        ...
        filename = self.artifactory.download_proper_version(image)
    ...
    def cache_pods_info(self):
        ...
        for node in pods_info:
            ...
            disconnected = f", status:'{node['connectionState']}'" \
    ...
    def get_radio_temperatures(self, radio: Union[int, str, list] = None, retries=3, **kwargs):
        ...
        if radio is None:
            return [self.get_radio_temperatures(value, retries=retries, **kwargs)
                    for _, value in radio_band_mapping.items()]
    ...
    def check_traffic_acceleration_flex(self, ip_address, wds=False, **kwargs):
        """
        Check if traffic was accelerated for flex connection
        Args:
            ip_address: (list) IP addresses to check
            wds: (bool) is wds enabled
            **kwargs:
    ...
    def check_traffic_acceleration(self, ip_address, expected_protocol=6, multicast=False, flow_count=1, **kwargs):
        """
    ...
    def kill_manager(self, wait_for_restart=False, time_to_restart=MANAGER_RESTART_TIMEOUT, **kwargs):
        """
        ...
        for manager in active_managers:
            ...
            log.info(f'Killing process for {manager} manger: kill -segv {initial_pid}')
            self.run_command(f'kill -segv {initial_pid}')
    ...
    def get_managers_list(self, managers_name: Union[str, list] = None, **kwargs):
        ...
        for manager_path in manager_paths:
            matched_processes = self.get_stdout(
                self.run_command(f'ps | grep {manager_path}', **kwargs), skip_exception=True).split('\n')[:-1]
    ...
    def recover(self, **kwargs):
        ...
        try:
            ...
            recover_path = ObjectResolver.resolve_model_path_file(file_name='recover/recover.py', model=model,
                                                                  wifi_vendor=default_family)
    ...
    def get_region(self, **kwargs):
        cc_codes = self.ovsdb.get_str('Wifi_Radio_State', 'country', return_list=True, skip_exception=True)
        if not cc_codes:
        ...
        for cc_code in cc_codes:
            if not cc_code:
                ...
                continue
            cc_code = 'EU' if cc_code.strip() == 'E0' else cc_code.strip()
            if cc_code != valid_cc:
    ...
    def _update_pod_ca_file(self, **kwargs):
        ...
        with open(ca_cert, 'r') as f:
            ...
            if ca_cert not in certs:
                ...
                for cmd in (f'cp {ca_path} {tmp_cas}',
    ...
    def redirect_stats_to_local_mqtt_broker(self, skip_storage=False, **kwargs):
        ...
        self.run_command('firewall-cli filter -m nat')
    ...
    def simulate_clients(self, count=1, **kwargs):
        if len(self.capabilities.get_lan_ifaces()) == 1:
            raise OpenSyncException('Device does not support simulating ethernet clients',
                                    'Use device with two Ethernet ports')
        device_types = [
            {"id": "lg-smarttv", "hostname": "LGSmartTV-558", "mac_prefix": "64:bc:0c",
             "fingerprint": "252,3,42,15,6,1,12"},
            {"id": "google-chromecast-ultra", "hostname": "Chromecast-Ultra-73", "mac_prefix": "3c:28:6d",
             "fingerprint": "1,33,3,6,15,28,51,58,59"},
            {"id": "microsoft-xbox", "hostname": "Xbox-SystemOS", "mac_prefix": "4c:0b:be",
             "vendor_class_id": "MSFT 5.0"},
            {"id": "microsoft-xbox", "hostname": "HOMEXBOX", "mac_prefix": "58:82:a8",
             "vendor_class_id": "MSFT 5.0"},
            {"id": "whatever", "hostname": "retropie-71", "mac_prefix": "b8:27:eb",
        ...
        def generate_random_mac(prefix):
            return f"{prefix}:{random.randint(0,9)}{random.randint(0,9)}:" \
        ...
        is_gw = False
        free_port = ''
        for eth in self.ovsdb.get_json_table('Connection_Manager_Uplink', where="if_type==eth"):
        ...
        # get used IPs so far
        out = [0, '', '']
        ips = self.ovsdb.get_str('DHCP_leased_IP', 'inet_addr', return_list=True)
        for _ in range(count):
            ...
            ret = self.ovsdb.set_value(table='OVS_MAC_Learning', value={"brname": "br-home", "hwaddr": new_mac,
                                                                        "ifname": free_port, "vlan": 0})
            out = self.merge_result(out, ret)
            ret = self.ovsdb.set_value(table='DHCP_leased_IP',
                                       value={"hostname": device['hostname'], "hwaddr": new_mac, "inet_addr": new_ip,
                                              "lease_time": 720000, "fingerprint": device.get('fingerprint', ''),
    ...
    def start_wifi_blast(self, plan_id=None, blast_duration=None, blast_packet_size=None, blast_sample_count=None,
                         mac_list: list = None, threshold_cpu=None, threshold_mem=None, **kwargs):
        ...
        result = self.ovsdb.set_value_wifi_blast(table='Wifi_Blaster_Config',
                                                 value={'plan_id': f'{plan_id}',
    ...
    def get_memory_information(self, **kwargs):
        ...
        mem_info_dict = {key: value.lower().translate({32: None})
        ...
        mem_info_dict = {key: int(value.split("kb")[0])
                         for key, value in mem_info_dict.items()}
    ...
    def configure_wifi_radio(self, freq_band: str, channel: int, ht_mode: str, **kwargs):
        ...
        """
        channel_request = self.ovsdb.set_value(value=dict(channel=channel), table='Wifi_Radio_Config',
                                               where=f'freq_band=={freq_band}', **kwargs)
        bw_request = self.ovsdb.set_value(value=dict(ht_mode=ht_mode), table='Wifi_Radio_Config',
                                          where=f'freq_band=={freq_band}', **kwargs)
...
class PodIface(Iface):
    ...
    def get_bhal_60_mac_list(self, bhal_60_iface=None):
        ...
        bhal_60_mac_list = self.lib.ovsdb.get_str(table='Wifi_VIF_State',
                                                  select='mac_list',
    ...
    def get_bhal_24_ssid(self):
        ...
        ssid_24 = self.lib.ovsdb.get_str(table='Wifi_VIF_State',
                                         select='ssid',
                                         where=[f'if_name=={bhaul_ap_24}', 'ssid_broadcast==disabled'])
        return ssid_24
    ...
    def get_all_allowed_channels_for_regulatory_domain(self, band_type):
        allowed_channels = self.lib.ovsdb.get_set(table='Wifi_Radio_State',
    ...
    def get_backhauls(self):
        ...
        for bhal_60_iface in bhal_60_ifaces:
            ...
            if associated_clients is not None:
                associated_clients = [str(assoc_client) for assoc_client in associated_clients
                                      if assoc_client is not None]
            bhal_dict[bhal_60_iface] = {'dev_if_name': bhal_60_iface, 'ssid': bhal_60_ssid, 'mac_list': mac_list,
                                        'associated_clients': associated_clients}
        return bhal_dict
    ...
    def get_all_home_bhaul_ifaces(self):
        ...
        all_interfaces = self.lib.capabilities.get_bhaul_ap_ifnames(
            return_type=list) + self.lib.capabilities.get_home_ap_ifnames(return_type=list)
...
class Ovsdb:
    ...
    def get(self, table, select: str, where: Union[str, list] = None, skip_exception=False):
        results = self.lib.get_stdout(self.get_raw(table, select, where), skip_exception=skip_exception)
        ...
        return self.lib.get_stdout(self.get_raw(table, select, where), skip_exception=skip_exception)
    ...
    def get_json_table(self, table, where: Union[str, list] = None, **kwargs):
        result = self.lib.get_stdout(self.lib.run_command(f'ovsh -j s {table} {self.generate_where(where)}', **kwargs),
                                     **kwargs)
        if not result:
            return None
    ...
    # TOD0: merge with the default set_value method once gte possibility to test
    ...
    def mutate(self, table: str, select: str, action: str, value: str, where: Union[str, list] = None,
               row_type: str = 'set', skip_exception=False):
    ...
    def generate_values_str(self, values, skip_exception=False, **kwargs):
        ...
        return " ".join([f'{k}{operator if operator else "~" if isinstance(v, str) else ":"}'
                         f'={self.python_value_to_ovsdb_value(v, skip_exception)}' for k, v in values.items()])
