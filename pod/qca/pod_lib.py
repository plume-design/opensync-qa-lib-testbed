...
class PodLib(PodLibGeneric):
    ...
    def get_radios_interference(self, **kwargs):
        ...
        for radio in self.capabilities.get_phy_radio_ifnames(return_type=list):
            survey = self.run_plume_or_opensync_stats_extra(f"plume {radio} survey_bss | "
                                                            f"awk '/raw/ {{print 100*($10-b)/($NF-t); b=$10; t=$NF; }}'"
                                                            , **kwargs)
    ...
    def trigger_crash(self, **kwargs):
        ...
        result = self.run_command(f"sh -c 'sleep 3; insmod {self.get_node_deploy_path()}/crash_qca.ko' &", **kwargs)
        time.sleep(20)
    ...
    def get_connection_flows_dump(self, ip_address='', **kwargs):
        ...
        """
        if self.qsdk_version >= 0x1100:
            return self.get_connection_flows_ecm_dump(ip_address, **kwargs)
        return self.get_connection_flows_sfe_dump(ip_address, **kwargs)
    ...
    def get_connection_flow_marks(self, ip_address, **kwargs):
        ...
        """
        if self.qsdk_version >= 0x1100:
            return self.get_connection_flow_marks_ecm(ip_address, **kwargs)
        return self.get_connection_flow_marks_sfe(ip_address, **kwargs)
    ...
    def check_traffic_acceleration(self, ip_address, expected_protocol=6, multicast=False, flow_count=1, **kwargs):
        ...
        if self.qsdk_version >= 0x1100:
            ecm_ok = self.check_traffic_acceleration_ecm(ip_address, expected_protocol=expected_protocol,
                                                         multicast=multicast, flow_count=flow_count, **kwargs)
            if not ecm_ok:
                return False
        return self.check_traffic_acceleration_sfe(ip_address, expected_protocol=expected_protocol,
                                                   multicast=multicast, flow_count=flow_count, **kwargs)
    ...
    def get_connection_flow_marks_sfe(self, ip_address, **kwargs):
        """
        Get connection flow marks for provided IP address
        Args:
            ip_address: (str) IP address
        ...
        """
        ip_connection_flows = self.get_connection_flows_sfe_dump(ip_address=ip_address, **kwargs)
        flow_marks = list()
        for ip_connection_flow in ip_connection_flows:
            flow_marks.append(ip_connection_flow.get('@dest_dscp', 0))
            flow_marks.append(ip_connection_flow.get('@src_dscp', 0))
        return flow_marks

    def check_traffic_acceleration_sfe(self, ip_address, expected_protocol=6, multicast=False, flow_count=1, **kwargs):
        """
        Check traffic was accelerated
        Args:
            ip_address: (list) IP addresses to check
        ...
        for i in range(5):
            ...
            time.sleep(4)
        assert all_connection_flows, f'Not found any connection flows for {ip_address} address, ' \
        ...
        for connection_flow in all_connection_flows:
            ...
            for key_to_check in ['@src_rx_pkts', '@src_rx_bytes', '@dest_rx_pkts', '@dest_rx_bytes']:
                ...
                if not value:
                    log.error(f'Value of {key_to_check} did not increased of the connection flow,'
                              f' for {self.get_nickname()} device')
                    status = False
        return status
    ...
    def get_connection_flows_ecm_dump(self, ip_address='', **kwargs):
        ...
        for connection_id in all_connection_ids:
            connection_flow = '\n'.join([flow_entry for flow_entry in connection_flows.splitlines()
                                         if connection_id in flow_entry])
    ...
    def get_connection_flow_marks_ecm(self, ip_address, **kwargs):
        """
        Get connection flow marks for provided IP address
        Args:
            ip_address: (str) IP address
        ...
        """
        ip_connection_flows = self.get_connection_flows_ecm_dump(ip_address=ip_address, **kwargs)
        flow_marks = list()
        for ip_connection_flow in ip_connection_flows:
            connection_mark = re.search(r'(?<=flow_dscp=)\d+', ip_connection_flow)
            if not connection_mark:
                continue
            flow_marks.append(connection_mark.group())
        return flow_marks

    def check_traffic_acceleration_ecm(self, ip_address, expected_protocol=6, multicast=False, flow_count=1, **kwargs):  # noqa: C901, E501
        """
        Check traffic was accelerated
        Args:
        ...
        if max_pr_accel < flow_count:
            log.error(f"There were less than {flow_count} flows where all the pr.accel classifiers were 'wanted': "
                      f"{max_pr_accel}")
            status = False
