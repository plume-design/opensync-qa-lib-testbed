...
class PowerControllerLib:
    ...
    def init_rpower_units(self):
        ...
        for rpower_unit in self.tb_config['rpower']:
            server_address, user, password, port = rpower_unit['ipaddr'], rpower_unit['user'], rpower_unit['pass'],\
                                                   rpower_unit['port']
            device_port = list()
            for device in rpower_unit['alias']:
                if device.get('port') is None:
                    raise OpenSyncException('Can not load rpower library without PDU ports.',
                                            'Make sure PDU ports are configured')
            ...
            pdu_type = self.get_pdu_type(rpi_server=server_object, ipaddr=server_address, username=user,
                                         password=password, port=port, pdu_ports=pdu_ports)
            ...
            pdu_lib = self.get_pdu_lib(pdu_type=pdu_type)(server_object=server_object, rpower_devices=rpower_devices,
                                                          pod_names=pod_names, client_names=client_names,
                                                          address=server_address, user=user, password=password,
                                                          port=port, tb_config=self.tb_config)
    ...
    def get_pdu_pod_names(self, rpower_devices):
        return [device_name for device_name in self.get_device_names(device_type='Nodes')
                if device_name in rpower_devices.keys()]

    def get_pdu_client_names(self, rpower_devices):
        return [device_name for device_name in self.get_device_names(device_type='Clients')
                if device_name in rpower_devices.keys()]
    ...
    def get_pdu_type(rpi_server, ipaddr, username, password, port, pdu_ports):
        ...
        ret = rpi_server.run_raw(f'{RPOWER_TOOL} -a name --ip-address {ipaddr}:{port} --user-name {username}'
                                 f' --password {password}')
