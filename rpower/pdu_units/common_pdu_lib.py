...
class CommonPduLib:
    def __init__(self, server_object, rpower_devices, pod_names, client_names, address, user, password, port,
                 tb_config, **kwargs):
        self.server_object, self.rpower_devices, self.pod_names, self.client_names, self.address, self.user, \
            self.password, self.port, self.tb_config = server_object, rpower_devices, pod_names, client_names, address,\
            user, password, port, tb_config
...
class PduLib(CommonPduLib):
    ...
    def port_args(self, device_names):
        ports_to_execute = [str(self.get_pdu_device_port(device_name)) for device_name in
                            self.get_devices_to_execute(device_names)]
        return ','.join(ports_to_execute)

    def execute_request(self, ports, action_name, args=''):
        if not ports:
            return 0, '', ''
        args += f' --pdu-type {self.get_name()} --user-name {self.user} --password {self.password} ' \
        ...
        for device_name in device_names:
            ...
            for stdout_port in response_stdout:
                if str(pdu_device_port) not in stdout_port:
                    continue
                results[device_name] = [0, stdout_port, ''] if 'failed' not in stdout_port.lower() else [1, '',
                                                                                                         stdout_port]
                break
        return results
