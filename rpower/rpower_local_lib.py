# IMPORTANT: We need to keep it as one of our customer do not use our testbed-server FW
    ...
    def execute_request(self, request, retry=3, **kwargs):
        ...
        while retry > 0:
            try:
                ...
                if not skip_logging:
                    log.warning(f"Unable to execute request: {url}")
                err_str = str(e)
            time.sleep(0.5)
            retry -= 1
        return [ret_val, std_out, err_str]
...
class CyberPowerController(PowerController):
    ...
    def __init__(self, address, port, user, passwd):
        ...
        kwargs = {"config": config, "multi_obj": True, "nicknames": ["host"], "skip_logging": True}
...
def get_config_item(container, key):
    if key not in container:
        raise OpenSyncException(
            "Power control unit not properly configured", f"Check rpower {key} settings in locations config"
        )
...
def rpower_init_config(config, **kwargs):
    ...
    for device in _rpower_config[RPOWER_DEVICE]:
        ...
        if "type" in device:
            ...
            for pdu in [
                WPSPowerController(ipaddr, port, user, passwd),
                CyberPowerController(ipaddr, port, user, passwd),
                EnergeniePowerController(ipaddr, port, user, passwd),
            ]:
                model = pdu.model(retry=3, skip_logging=True, timeout=60)
                if model[0] == 0:
                    power_controller = pdu
                    break
            else:
                raise Exception(f"Cannot detect remote PDU model: {ipaddr}:{port}")
