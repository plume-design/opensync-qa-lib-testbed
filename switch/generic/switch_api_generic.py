...
class SwitchApiGeneric:
    ...
    def setup_class_handler(self, request):
        if not request or not request.config or request.config.option.skip_init:
            return
    ...
    def get_device_port_names(self):
        ...
        for device in self.config["Nodes"]:
            port_names = [
                port_name
                for port_name, port_number, vlan_number in self.get_all_switch_aliases(device["name"])
                if port_name
            ]
    ...
    def get_role_of_port(self, vlan_port_name):
        ...
        """
        vlan_roles = [
            vlan_role["switch"][vlan_port_name]
            for vlan_role in self.config["Nodes"]
    ...
    def disable_no_used_ports(self, dev_name=None, device_aliases=None, vlan_type="untagged"):
        ...
        if None not in switch_aliases and len(switch_aliases) >= 1:
            ...
            use_port = [
                port_name
                for port_name in port_names
                if not use_port and self.get_wan_vlan_number(self.switch_info(port_name), vlan_type=vlan_type)
            ]
            if use_port:
                use_port = use_port[0]
    ...
    def switch_wan_port(
        ...
        Args:
            ...
            ignore_missing: (bool) Don't assert if pod is without any vlan_type WAN VLAN
        ...
        default_old_wan_blackhole = [
            port_alias[2] for port_alias in port_aliases if old_wan_port_name == port_alias[0]
        ][0]
    ...
    def connect_eth_client(self, pod_name, client_name, connect_port="", **kwargs):
        ...
        # get eth client vlan
        client_blackhole = [
            client.get("vlan") for client in self.config.get("Clients") if client_name == client.get("name")
        ][0]
        ...
        for port in port_info:
            ...
            break
        assert target_port, (
            "Target port has not been found. Probably you want to connect eth client to wan port or "
    ...
    def disconnect_all_pods_from_client_vlan(self, client_blackhole_vlan, disable_unused_ports=True):
        """
        ...
        Args:
            ...
            disable_unused_ports: (bool) Should pod switch ports get disabled, if not also used for other purposes
        ...
        for alias_info in all_alias_infos:
            ...
            if client_blackhole_vlan == port_info["pvid"] or client_blackhole_vlan in port_info["untagged"]:
                ...
                disconencted_aliases.append(alias_info)
        if disable_unused_ports:
            self.disable_no_used_ports(device_aliases=disconencted_aliases)
    ...
    def get_wan_port(self, pod_name, ignore_missing=False):
        """
        Get wan port from target device
        Args:
            pod_name: (str) name of device
            ignore_missing: (bool) Don't assert if pod is without wan port
        ...
        for port_name in ports_name:
            ...
            if self.get_wan_vlan_number(port_info) or self.get_wan_vlan_number(port_info, vlan_type="tagged"):
                wan_port = port_name
                break
        if ignore_missing:
            return wan_port
    ...
    def remove_custom_tagged_vlans(self, pod_name, default_tagged_vlans=DEFAULT_TAGGED_VLANS):
        port_aliases = self.get_all_switch_aliases(pod_name)
        assert port_aliases, f"Not found port information for {pod_name} device"
        ports_name = [port_name[0] for port_name in port_aliases]
        ports_info = self.switch_info(ports_name)
    ...
    def get_no_wan_port(self, switch_aliases):
        port_name, default_backhaul = "", ""
        for switch_alias in switch_aliases:
            vlan_port_name, number_port, backhaul = switch_alias
            assert vlan_port_name, (
                f"Can not describe vlan port name for {switch_aliases}. "
            ...
            if not wan_pvid:
                port_name, default_backhaul = vlan_port_name, backhaul
                break
    ...
    def get_devices_connection_type(self):
        ...
        for device_name in devices_name:
            ...
            for switch_alias in self.get_all_switch_aliases(device_name):
                port_name, port_number, backhaul_number = switch_alias
    ...
    def get_default_port_vlan(self, port_name: str):
        for port_alias in self.switch_ctrl.alias:
            if port_alias["name"] != port_name:
                continue
