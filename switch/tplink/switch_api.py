...
PORT_15_CONF = {'name': PORT_15_NAME,
                'port': 15,
                'backhaul': 309}
...
class SwitchApi(SwitchApiGeneric):
    ...
    def add_tagged_vlan(self, port_name, target_vlan, enable_port=True):
        ...
        while True:
            self.switch_ctrl.interface_down(port_name)
            res = self.switch_ctrl.vlan_set(port_name, target_vlan, 'tagged')
            time.sleep(10)
            if enable_port:
                log.info(f'Enabling {port_name}')
                self.switch_ctrl.interface_up(port_name)
    ...
    def connect_eth_client(self, pod_name, client_name, connect_port='', **kwargs):
        ...
        """
        target_port = super().connect_eth_client(pod_name=pod_name, client_name=client_name,
                                                 connect_port=connect_port, **kwargs)
    ...
    def disconnect_eth_client(self, pod_name, client_name, disable_no_used_ports=True):
        ...
        """
        client_port = super().disconnect_eth_client(pod_name=pod_name, client_name=client_name,
                                                    disable_no_used_ports=disable_no_used_ports)
    ...
    def recovery_port_isolation_from_static_cfg(self):
        ...
        switch_ip = self.config['Switch'][0]['ipaddr'] if self.config['Switch'][0].get('ipaddr') else \
            self.config['Switch'][0].get('hostname')
        assert switch_ip, 'Can not determine IP address to the switch'
        system_info = self.get_stdout_switch_requests(self.switch_ctrl.get_system_info()).lower()
        assert system_info, 'Can not determine switch type'
        # init server obj to get switch cfg
        from lib_testbed.generic.client.client import Client
        kwargs = {'config': self.config, 'multi_obj': True, 'type': 'rpi', 'nickname': 'host'}
        client_obj = Client(**kwargs)
        server = client_obj.resolve_obj(**kwargs)
        cfg_config = None
        # Determine switch cfg
        if 't1600g-18ts' in system_info:
            cfg_config = os.path.join(SWITCH_CFG_DIR, CFG_16_PORTS)
        elif 'sg2218' in system_info:
            cfg_config = os.path.join(SWITCH_CFG_DIR, CFG_16_PORTS_V2)
        elif 't1500g-8t' in system_info:
            cfg_config = os.path.join(SWITCH_CFG_DIR, CFG_8_PORTS)
        if not cfg_config:
            log.warn('Can not determine switch type. Skip recovery port isolation...')
            return False
        # get switch config
        switch_cfg = server.run(f'cat {cfg_config}', skip_exception=True)
        assert switch_cfg, f'Can not get switch config: {cfg_config} from testbed server'
        ...
        for port_name in self.get_list_of_all_port_names():
            ...
            if not port_alias:
                log.warn(f'Can not get port alias for {port_name}')
                continue
            port_number = str(port_alias['port'])
            default_port_isolation = isolation_config.get(port_number)
            if not default_port_isolation:
                log.warn(f'Can not describe isolation config for {port_name} and port number: {port_number}')
                continue
