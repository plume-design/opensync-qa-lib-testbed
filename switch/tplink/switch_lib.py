...
class SwitchLib(SwitchLibGeneric, spawn):
    def __init__(self, switch_name, user, password, ip, port, set_echo=False, **kwargs):
        ...
        spawn.__init__(self, command=None, timeout=REQUEST_TIMEOUT, maxread=MAX_READ, searchwindowsize=None,
                       logfile=None, cwd=None, env=None)
        SwitchLibGeneric.__init__(self, switch_name=switch_name, user=user, password=password, ip=ip, port=port,
                                  **kwargs)
    ...
    def login(self):
        ...
        for _ in range(10):
            self.send_command("")
            if isinstance(self.after, bytes) and (self.after.endswith(b'>') or self.after.endswith(b'#')) or \
    ...
    def restore_config(self):
        def get_client_host():
            ...
            kwargs = {'config': copy.deepcopy(self.tb_config), 'nickname': 'host', 'multi_obj': False}
        ...
        while time.time() < timeout:
            if client.run_raw('pwd', skip_logging=True)[0] == 0:
                break
            time.sleep(5)
        else:
